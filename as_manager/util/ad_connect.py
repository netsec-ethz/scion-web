...
from as_manager.models import (
    ...
    SibraServerWeb,
    AS)
from as_manager.util.common import is_private_address
...
def link_ases(first_as, second_as, link_type):
    """Needs transaction!"""
    assert isinstance(first_as, AS)
    assert isinstance(second_as, AS)
    first_topo = first_as.generate_topology_dict()
    second_topo = second_as.generate_topology_dict()
    first_topo, second_topo = link_topologies(first_topo, second_topo,
                                              link_type)

    first_as.fill_from_topology(first_topo, clear=True)
    second_as.fill_from_topology(second_topo, clear=True)
...
def create_new_as_files(parent_ad_topo, isd_id, ad_id, out_dir):
