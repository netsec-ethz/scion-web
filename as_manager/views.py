...
from as_manager.util.response_handling import (
    get_failure_errors,
    get_success_data,
    is_success,
)
from as_manager.util.util import to_b64, from_b64
...
from as_manager.forms import (
    ConnectionRequestForm,
    CoordinationServiceSettingsForm,
    NewLinkForm,
    UploadFileForm
)
from as_manager.models import AS, ISD, ConnectionRequest,\
    OrganisationAdmin, User, JoinRequest
from as_manager.util.ad_connect import (
    create_new_as_files,
    find_last_router,
    # link_ases,
)
from as_manager.util.errors import HttpResponseUnavailable
...
from as_manager.util.hostfile_generator import generate_ansible_hostfile
...
class ISDDetailView(ListView):
    model = AS
    template_name = 'as_manager/isd_detail.html'
    paginate_by = 20
    ...
    def get_queryset(self):
        ...
        self.isd = isd
        queryset = isd.as_set.all().order_by('id')
        return queryset
...
def add_as(request):
    ...
    try:
        ...
        as_obj = AS.objects.create(as_id=as_id, isd=isd,
                                   is_core_as=False,
                                   is_open=False)
        as_obj.save()
    as_obj = AS.objects.get(as_id=as_id, isd=current_isd)
...
def accept_join_request(request, isd_as, request_id):
    ...
    core_as_id = isd_as
    core_as = AS.objects.get(as_id=isd_as.split('-')[1], isd=isd_as.split('-')[0])
...
class ADDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        as_obj = context['object']

        # Status tab
        context['routers'] = as_obj.routerweb_set.select_related()
        context['path_servers'] = as_obj.pathserverweb_set.all()
        context['certificate_servers'] = as_obj.certificateserverweb_set.all()
        context['beacon_servers'] = as_obj.beaconserverweb_set.all()
        context['sibra_servers'] = as_obj.sibraserverweb_set.all()

        context['management_interface_ip'] = get_own_local_ip()
        context['reloaded_topology'] = as_obj.original_topology
        flat_string = json.dumps(as_obj.original_topology, sort_keys=True)
        ...
        context['as_id'] = as_obj.as_id
        context['isd_id'] = as_obj.isd_id
        isdas = '-'.join([str(as_obj.isd_id), str(as_obj.as_id)])
        context['isdas'] = isdas
        ...
        # Permissions
        context['user_has_perm'] = self.request.user.has_perm('change_ad',
                                                              as_obj)
        return context
...
def as_topo_hash(request, isd_id, as_id):
    try:
        as_obj = AS.objects.get(as_id=as_id,
                            isd=isd_id)
        flat_string = json.dumps(as_obj.original_topology, sort_keys=True)
        # hash for non cryptographic purpose (state comparison for user warning)
...
def _check_user_permissions(request, as_obj):
    # TODO(rev112) decorator?
    if not request.user.has_perm('change_ad', as_obj):
...
def control_process(request, pk, proc_id):
    ...
    """
    as_obj = get_object_or_404(AS, id=pk)  # load by as_id
    _check_user_permissions(request, as_obj)

    ad_elements = as_obj.get_all_element_ids()
    ...
    response = run_remote_command(as_obj.md_host, proc_id, command,
                                  use_ansible=False)
...
def read_log(request, pk, proc_id):
    ...
    as_obj = get_object_or_404(AS, id=pk)  # TODO: query by as_id
    _check_user_permissions(request, as_obj)

    ad_elements = as_obj.get_all_element_ids()
    if proc_id not in ad_elements:
        return HttpResponseNotFound('Element not found')
    proc_id = as_obj.get_full_process_name(proc_id)

    response = run_remote_command(as_obj.md_host, proc_id, 'STATUS',
                                  use_ansible=False)
    if is_success(response):
...
class ConnectionRequestView(FormView):
    ...
    template_name = 'as_manager/new_connection_request.html'
    success_url = ''
    ...
    def form_valid(self, form):
        ...
        conn_request.connect_from = get_object_or_404(AS, id=connect_from)
        ...
        key = coord_settings.key + "/"
        secret = coord_settings.secret
        isd_as = '-'.join([str(self._get_as().isd.id),
                           str(self._get_as().as_id)])
        ...
        connection_request_dict["certificate"] = self._get_as().certificate
        base_url = COORD_SERVICE_URI
    ...
    def get_context_data(self, **kwargs):
        ...
        context_data['ad'] = self._get_as()
        return context_data


class NewLinkView(FormView):
    form_class = NewLinkForm
    template_name = 'as_manager/new_link.html'
    success_url = ''

    def _get_as(self):
        if not hasattr(self, 'ad'):
            self.as_obj = get_object_or_404(AS, id=self.kwargs['pk'])
        return self.as_obj
    ...
    def form_valid(self, form):
        ...
        #     link_ases(from_ad, to_ad, link_type)

        self.success_url = reverse('as_detail', args=[this_as.id])
        return super().form_valid(form)


def approve_request(as_obj, ad_request):
    # Create the new AS
    new_id = AS.objects.latest('id').id + 1
    new_as = AS.objects.create(as_id=new_id, isd=as_obj.isd,
                               md_host=ad_request.router_public_ip)
    parent_topo_dict = as_obj.generate_topology_dict()

    with tempfile.TemporaryDirectory() as temp_dir:
        new_topo_dict, parent_topo_dict = create_new_as_files(parent_topo_dict,
                                                              new_as.isd.id,
                                                              new_as.id,
                                                              out_dir=temp_dir)
        ...
        new_as.fill_from_topology(new_topo_dict, clear=True)
        as_obj.fill_from_topology(parent_topo_dict, clear=True)

        # Update the new topology on disk:
        # Write new config files to disk, regenerate everything else
        # FIXME(rev112): minor duplication, see ad_connect.create_new_as_files()
        gen = ConfigGenerator(out_dir=temp_dir)
        new_topo_path = gen.path_dict(new_as.isd.id, new_as.id)['topo_file_abs']
        ...
        package_dir = os.path.join('gen', 'AS' + str(
            new_as))  # os.path.join(PACKAGE_DIR_PATH,
        # 'AS' + str(new_as)) TODO: replace ad_management functionality
        if os.path.exists(package_dir):
            rmtree(package_dir)
        os.makedirs(package_dir)

        # Prepare package
        # package_name = 'scion_package_AD{}-{}'.format(new_as.isd, new_as.id)
        ...
        ad_request.new_as = new_as
        ad_request.status = 'APPROVED'
        ad_request.save()

        # Give permissions to the user
        request_creator = ad_request.created_by
        assign_perm('change_ad', request_creator, new_as)
...
def request_action(request, req_id):
    ...
    replying_as = get_object_or_404(AS, isd=isd_id, as_id=as_id)
...
def list_sent_requests(request):
    ...
    return render(request, 'as_manager/sent_requests.html',
                  {'sent_requests': sent_requests,
                   'received_replies': new_received_replies}
                  )


@permission_required('as_manager.change_organisationadmin', login_url='/login/')
def coord_service(request):
    ...
    return render(request, 'as_manager/coord_service.html',
                  {'settings_form': form})
...
def _get_partial_graph(pov_as, rank=1):
    partial_graph = {}
    bfs_queue = deque([[pov_as, rank]])
    while bfs_queue:
        next_as, ad_rank = bfs_queue.popleft()
        if next_as in partial_graph:
            continue

        ad_routers = next_as.routerweb_set.all().select_related('neighbor_as')
        neighbors = []
        for router in ad_routers:
            neighbor_as = router.neighbor_as
            if ad_rank > 0:
                bfs_queue.append([neighbor_as, ad_rank - 1])
...
def _get_node_object(as_obj):
    node_object = {
        'name': 'AS {}-{}'.format(as_obj.isd_id, as_obj.id),
        'group': as_obj.isd_id,
        'url': as_obj.get_absolute_url(),
        'networkUrl': reverse('network_view_ad', args=[as_obj.id]),
        'core': int(as_obj.is_core_as),
    }
    return node_object


def network_view_neighbors(request, pk):
    pov_as = get_object_or_404(AS, id=pk)  # query by as_id
    rank = 2

    partial_graph = _get_partial_graph(pov_as, rank)
    ad_with_neighbors = partial_graph.keys()

    # Build reverse index
    ad_index_rev = {}
    for i, as_obj in enumerate(ad_with_neighbors):
        ad_index_rev[as_obj] = i

    graph = {'nodes': [], 'links': []}
    for as_obj in ad_with_neighbors:
        index = ad_index_rev[as_obj]
    ...
    return render(request, 'as_manager/network_view.html',
                  {'data': graph,
                   'pov_as': pov_as})
...
def network_view(request):
    ...
    """
    all_ases = AS.objects.all().prefetch_related('routerweb_set__neighbor_as')
    ad_graph_tmp = []
    ...
    for i, as_obj in enumerate(all_ases):
        ad_index[i] = as_obj
        ad_index_rev[as_obj] = i
        ad_routers = as_obj.routerweb_set.all()
        ad_graph_tmp.append([r.neighbor_as for r in ad_routers])
    ...
    return render(request, 'as_manager/network_view.html', {'data': graph})
...
static_tmp_path = os.path.join(WEB_ROOT, 'as_manager', 'static', 'tmp')
...
def generate_topology(request):
    ...
    curr_as = get_object_or_404(AS, as_id=as_id, isd=isd_id)
